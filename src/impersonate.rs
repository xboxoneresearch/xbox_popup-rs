...
impl Impersonate {
    ...
    pub fn do_impersonate(&mut self, process_name: &str) -> Result<(), Box<dyn std::error::Error>> {
        self.enable_debug_privilege().or_else(|e| {
            println!("Failed to enable debug privilege, err: {e:?}");
            Err(e)
        })?;
        ...
        for p in process_list::get_process_list()? {
            ...
            if p.name.to_lowercase() == process_name.to_lowercase() {
                pid = p.id;
                println!("Found PID for {process_name} -> {pid}");
            }
        }
